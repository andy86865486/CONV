...
// 座標計數器 (0 ~ 63)
reg [5:0] x, y, target_col, target_row; 
...
localparam S_FILL_B = 4'd4;
localparam S_L0 = 4'd5;
localparam S_FILL_4 = 4'd6;
localparam S_L1 = 4'd7;
...
always @(*) begin
	...
	S_FILL_B : begin
	if (r_cnt == 3'd3) next_state = S_L0;
	else next_state = S_FILL_B;
	end
	
	S_L0 : begin
	if (x == 6'd0 && y == 6'd63) next_state = S_FILL_4;
	else if ((x == 6'd63) && (y[0] == 1'b0)) next_state = S_FILL_B;
	else if ((x == 6'd0) && (y[0] == 1'b1)) next_state = S_FILL_B;
	else if (y[0] == 1'b1) next_state = S_FILL_L;
	else if (y[0] == 1'b0) next_state = S_FILL_R;
	end
...
always @(*) begin
	...
	S_FILL_B : target_row = y + 1'b1;
	default: begin
		target_col = x; target_row = y;
		end
	endcase
end
...
always @(posedge clk or posedge reset) begin
	...
	3'd2 : begin
		iaddr <= {target_row, (x + 1'b1)};
		if (y == 6'd63) w3[1][2] <= 20'sd0;
		else w3[1][2] <= idata ;
		end
	3'd3 : begin
		if (x == 6'd63 || y == 6'd63) w3[2][2] <= 20'sd0;
	...
	if ((y == 6'd63) && (x == 6'd0)) begin
		x <= 6'd0;
		y <= 6'd0;
		end
	else if ((x== 6'd63) && (y[0] == 1'b0)) begin
		x <= 6'd63;
		y <= y + 1'b1;
		{w3[0][0], w3[1][0], w3[2][0]} <= {w3[0][1], w3[1][1], w3[2][1]};
		{w3[0][1], w3[1][1], w3[2][1]} <= {w3[0][2], w3[1][2], w3[2][2]};
		end
	else if ((x == 6'd0) && (y[0] == 1'b1)) begin
		x <= 6'd0;
		y <= y + 1'b1;
		{w3[0][0], w3[1][0], w3[2][0]} <= {w3[0][1], w3[1][1], w3[2][1]};
		{w3[0][1], w3[1][1], w3[2][1]} <= {w3[0][2], w3[1][2], w3[2][2]};
		...
		x <= x - 1'b1;
		{w3[1][0], w3[2][0]} <= {w3[0][0], w3[1][0]};
		{w3[1][1], w3[2][1]} <= {w3[0][1], w3[1][1]};
		{w3[1][2], w3[2][2]} <= {w3[0][2], w3[1][2]};
		end
	end
